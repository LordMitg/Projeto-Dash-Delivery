...
import { StockAdvancedControl } from './StockAdvancedControl'
...
export const IngredientsManagement = () => {
  ...
  const [tab, setTab] = useState<'stock' | 'movements' | 'lots' | 'inventory'>('stock')
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-5">
      ...
      <nav className="flex overflow-x-auto border-b border-line">
        ...
        <button type="button" onClick={() => setTab('lots')} className={`inline-flex min-w-fit items-center gap-1.5 border-b-2 px-4 py-3 text-sm font-semibold ${tab === 'lots' ? 'border-brand text-accent' : 'border-transparent text-slate'}`}><CalendarDays className="h-4 w-4" />Lotes e validade</button>
        <button type="button" onClick={() => setTab('inventory')} className={`inline-flex min-w-fit items-center gap-1.5 border-b-2 px-4 py-3 text-sm font-semibold ${tab === 'inventory' ? 'border-brand text-accent' : 'border-transparent text-slate'}`}><ClipboardCheck className="h-4 w-4" />Inventário</button>
      </nav>
      ...
      {tab === 'stock' ? (
        ...
        <StockAdvancedControl mode={tab} ingredients={ingredients} canManage={canManage} onChanged={load} />
      )}
